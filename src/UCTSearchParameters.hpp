...
{											        // DEFAULT				DESCRIPTION
    ...
    size_t          m_rootMoveSelection;             // MostVisited          Which node to pick from the root

	size_t		    m_timeLimit;					    // 0					Search time limit. 0 means no time limit
    double          m_cValue;                        // 1                    C constant for UCT formula
    size_t          m_maxTraversals;                 // 100                  Max number of UCT traversals to make
...
public:
	...
	UCTSearchParameters() 
        ...
        , m_rootMoveSelection    (UCTMoveSelect::MostVisited)
	    , m_timeLimit            (0)
        , m_cValue               (1)
        , m_maxTraversals        (100)
        , m_maxChildren          (10)
    ...
    const double & cValue()							            const   { return m_cValue; }
    const size_t & maxTraversals()						        const   { return m_maxTraversals; }
    ...
    const size_t & rootMoveSelectionMethod()                    const   { return m_rootMoveSelection; }
    ...
    void setCValue(const double & c)					                { m_cValue = c; }
    void setMaxTraversals(const size_t & traversals)                    { m_maxTraversals = traversals; }
    ...
    void setRootMoveSelectionMethod(const size_t & method)              { m_rootMoveSelection = method; }
    ...
    std::vector<std::vector<std::string> > & getDescription()
    {
        if (m_desc.size() == 0)
        {
            ...
            m_desc[0].push_back("C Value:");
            m_desc[0].push_back("Max Traversals:");
            ...
            ss << "UCT";                                                m_desc[1].push_back(ss.str()); ss.str(std::string());
            ss << timeLimit() << "ms";                                  m_desc[1].push_back(ss.str()); ss.str(std::string());
            ss << cValue();                                             m_desc[1].push_back(ss.str()); ss.str(std::string());
            ss << maxTraversals();                                      m_desc[1].push_back(ss.str()); ss.str(std::string());
